...
import { Button, EmptyState, Field, Input, Modal, PageHeader } from "@/components/ui";
...
function TeamDialog({ team, onClose }: { team: Team | null; onClose: (saved: boolean) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const res = await fetch(isNew ? "/api/teams" : `/api/teams/${team!.id}`, { method: isNew ? "POST" : "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: name.trim(), key: key.toUpperCase(), wipLimit: Number(wipLimit) }) });
    setBusy(false);
  ...
  return <Modal open onOpenChange={(open) => { if (!open) onClose(false); }} title={isNew ? "New team" : `Edit ${team!.name}`} description={isNew ? "A team owns cycles and can lead projects." : "Update the team's name, key and WIP limit."}>
